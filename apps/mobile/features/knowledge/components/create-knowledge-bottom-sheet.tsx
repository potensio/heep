...
interface CreateKnowledgeBottomSheetProps {
  onSubmit: (data: { location: Location; text: string }) => void;
}

export const CreateKnowledgeBottomSheet = forwardRef<
  CreateKnowledgeBottomSheetRef,
  CreateKnowledgeBottomSheetProps
>(({ onSubmit }, ref) => {
  ...
  const canSubmit = !!selectedLocation && text.trim().length > 0;
  ...
  const handleSubmit = useCallback(() => {
    ...
    onSubmit({ location: selectedLocation, text: text.trim() });
    setSelectedLocation(null);
    setText("");
    setQuery("");
    sheetRef.current?.close();
  }, [selectedLocation, text, onSubmit]);
  ...
  return (
    <BottomSheet ref={sheetRef} snapPoints={["68%"]}>
      <BottomSheetView style={styles.container}>
        ...
        {/* Knowledge text */}
        <View style={{ gap: 8 }}>
          <Text style={styles.label}>What should Heep remember?</Text>
          <BottomSheetTextInput
            style={styles.input}
            placeholder="e.g. We're closed on Mondays and open 6pm–11pm on weekends."
            placeholderTextColor="rgba(255,255,255,0.3)"
            value={text}
        ...
        >
          <Text style={styles.submitText}>Add to memory</Text>
        </Pressable>
      </BottomSheetView>
    </BottomSheet>
  );
...
CreateKnowledgeBottomSheet.displayName = "CreateKnowledgeBottomSheet";
